...
import { ArrowLeft, ArrowRight, Bell, Bot, Check, ChevronDown, Clock3, Code2, ExternalLink, Loader2, Play, Target, Trophy, X } from "lucide-react";
...
function AskFlow({participate,publishQuestion}:{participate:()=>void;publishQuestion:(question:string)=>Promise<{questions:any[];official_predictions:OfficialPrediction[]}>}){
  ...
  const confirm=async()=>{
    if(!isPublic){setStep("result");return}
    setOfficialPreds([]);
    setPredicting(true);
    setStep("result");
    const res=await publishQuestion(question);
    setOfficialPreds(res.official_predictions||[]);
    setPredicting(false);
  };
